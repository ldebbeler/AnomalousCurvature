...
int main(int argc, char** argv){
    ...
    std::size_t L{ 195 };
    std::vector<double> at(L);

    std::vector<double> apos(L);
    std::vector<double> aneg(L);
    std::vector<double> bpos(L);
    std::vector<double> bneg(L);
    std::vector<double> cpos(L);
    std::vector<double> cneg(L);
    std::vector<double> dpos(L);
    std::vector<double> dneg(L);

    std::vector<double> deltab(L);

    //#pragma omp parallel for schedule(dynamic)
    for(std::size_t i=0; i<at.size(); i++){
        double alpha{ 4.0-0.01*i };
        selfenergy se(alpha);
        ...
        bpos[i] = se.posSlope();
        bneg[i] = se.negSlope();
        cpos[i] = se.posQuadratic();
        cneg[i] = se.negQuadratic();
        dpos[i] = se.posQuartic();
        dneg[i] = se.negQuartic();

        seTang tangb(alpha);
        deltab[i] = tangb.deltab();
    }
    ...
    std::string co_file{ "data/coefficients_alpha.h5" };

    H5::H5File file(co_file, H5F_ACC_TRUNC );
    writeNew writefile(results);
    writefile.writeCoefficients(file);
    
    std::cout << "File created: \t" << co_file << '\n';
