...
writeh5::writeh5(const scalingValues& scaling, const bubbleValues& bubble, const selfEnergyValues& sev):
    m_scaling(scaling), m_bubble(bubble), m_selfEnergy(sev) {}

writeh5::writeh5(const scalingValues& scaling, const selfEnergyValues& sev): m_scaling(scaling), m_selfEnergy(sev) {}
...
// group and data for scaling functions
    ...
    write1dvector(file, "/ScalingFunction/FirstReal", m_scaling.m_firstReal);
    write1dvector(file, "/ScalingFunction/FirstImag", m_scaling.m_firstImag);
    write1dvector(file, "/ScalingFunction/SecondReal", m_scaling.m_secondReal);
    write1dvector(file, "/ScalingFunction/SecondImag", m_scaling.m_secondImag);
...
// group and data for bubble with real frequencies
    ...
    write1dvector(file, "/SelfEnergy/SEfreq", m_selfEnergy.m_SE);
    // uncomment for non empty set m_SE
    //write2dvector(file, "/SelfEnergy/SE", m_selfEnergy.m_SE);

    H5::Attribute arp = selfEnergy.createAttribute( "Aplus", double_type, att_space );
    arp.write( double_type, &m_selfEnergy.m_freqPlus);

    H5::Attribute arm = selfEnergy.createAttribute( "Aminus", double_type, att_space );
    arm.write( double_type, &m_selfEnergy.m_freqMinus);

    write1dvector(file, "/SelfEnergy/rads", m_selfEnergy.m_rads);
    write1dvector(file, "/SelfEnergy/radScalingPos", m_selfEnergy.m_radScalingPos);
    write1dvector(file, "/SelfEnergy/radScalingNeg", m_selfEnergy.m_radScalingNeg);
    write1dvector(file, "/SelfEnergy/tangs", m_selfEnergy.m_tangs);
    write1dvector(file, "/SelfEnergy/tangScalingPos", m_selfEnergy.m_tangScalingPos);
    write1dvector(file, "/SelfEnergy/tangScalingNeg", m_selfEnergy.m_tangScalingNeg);

    write1dvector(file, "/SelfEnergy/diff", m_selfEnergy.m_dkt);
    write1dvector(file, "/SelfEnergy/diffTangPos", m_selfEnergy.m_dTangPos);
    write1dvector(file, "/SelfEnergy/diffTangNeg", m_selfEnergy.m_dTangNeg);

    write1dvector(file, "/SelfEnergy/dt", m_selfEnergy.m_derivKt);
    write1dvector(file, "/SelfEnergy/FirstDerivPos", m_selfEnergy.m_derivTangPos);
    write1dvector(file, "/SelfEnergy/FirstDerivNeg", m_selfEnergy.m_derivTangNeg);
